...
const Meals = () => {
  ...
  const [noMealsNotified, setNoMealsNotified] = useState(false); // Track notification state
  const debounceTimeout = useRef(null); // Use ref for debounce to avoid re-renders
  ...
  const fetchMeals = useCallback(async () => {
    ...
    try {
      ...
      addNotification(`Failed to load meals: ${error.message}`, "error");
      setMeals([]);
      setTotalCount(0);
      setNoMealsNotified(false);
    } finally {
  ...
  }, [filters, skip, take, currentPage, noMealsNotified, addNotification]);
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <div className="fixed top-4 right-4 z-50">
        {/* Notification rendering handled by NotificationContext */}
      </div>
